...
import {
  craeteWorkspaceSchema,
  CreateWorkspaceSchemaFormValues,
} from "../types/validator";
import useConfirmDialog from "@/hooks/shared/useConfirmDialog";

const CreateWorkspaceForm = () => {
  const { onCloseDialog } = useConfirmDialog();
  const { mutate, isPending } = useCreateWorkspace({
    onClose: onCloseDialog || (() => {}),
  });

  const form = useForm<CreateWorkspaceSchemaFormValues>({
    resolver: zodResolver(craeteWorkspaceSchema),
    defaultValues: {
      name: "",
      description: "",
    },
  ...
  const onSubmit = (values: CreateWorkspaceSchemaFormValues) => {
    if (isPending) return;
    mutate(values);
  };
  ...
  return (
    <main className="w-full flex flex-row min-h-[590px] h-auto max-w-full">
      ...
      bg-[url('/images/hhh.jpg')] bg-cover bg-center h-full
      "
      />
    </main>
  );
